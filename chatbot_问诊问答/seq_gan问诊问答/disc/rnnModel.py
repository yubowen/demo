...
from disc import core_rnn_cell
...
class rnnModel(object):
    def __init__(self, config, name_scope, dtype=tf.float32):
        ...
        # 改：encoder_mutil = tf.contrib.rnn.MultiRNNCell([encoder_cell] * num_layers)
        encoder_mutil = tf.nn.rnn_cell.MultiRNNCell([encoder_cell] * num_layers)
        

        #rnn_layers = [tf.nn.rnn_cell.LSTMCell(size) for size in [128, 256]]

        # 改：encoder_emb = tf.contrib.rnn.EmbeddingWrapper(encoder_mutil, embedding_classes=vocab_size, embedding_size=emb_dim)
        encoder_emb = core_rnn_cell.EmbeddingWrapper(encoder_mutil, embedding_classes=vocab_size, embedding_size=emb_dim)         

        # 改：context_cell = tf.contrib.rnn.BasicLSTMCell(emb_dim)
        context_cell = tf.nn.rnn_cell.BasicLSTMCell(emb_dim)

        # 改：context_multi = tf.contrib.rnn.MultiRNNCell([context_cell] * num_layers)
