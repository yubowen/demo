...
from utils import bio_to_json,get_logger
...
class Model(object):
    def __init__(self, config):
        ...
        lstm_inputs = self.bert_bilstm(lstm_inputs)
        lstm_inputs = tf.nn.dropout(lstm_inputs, self.dropout)
        ...
        for var in tvars:
            ...
            # elif var.name.startswith("char_BiLSTM") or var.name.startswith("project/hidden"):
            #     init_string = ", *INIT_FROM_CKPT*"
        ...
        self.saver = tf.train.Saver(vl, max_to_keep=5)
        
        # 迁移restore bilstm+crf valueable
        bilstm_var = [v for v in train_vars if v.name.startswith("char_BiLSTM") or v.name.startswith("project/hidden")]
        self.bilstm_saver = tf.train.Saver(bilstm_var, max_to_keep=5)
    ...
    def bert_bilstm(self, bert_output, name=None):
        """
        hidden layer between lstm layer and logits
        :param lstm_outputs: [batch_size, num_steps, emb_size] 
        :return: [batch_size, num_steps, num_tags]
        """
        with tf.variable_scope("Embedding_BiLstm"  if not name else name):
            with tf.variable_scope("hidden"):
                W = tf.get_variable("W", shape=[768, 120],
                                    dtype=tf.float32, initializer=self.initializer)

                b = tf.get_variable("b", shape=[120], dtype=tf.float32,
                                    initializer=tf.zeros_initializer())
                output = tf.reshape(bert_output, shape=[-1, 768])
                hidden = tf.tanh(tf.nn.xw_plus_b(output, W, b))
                hidden = tf.reshape(hidden, shape=[-1, self.num_steps, 120])
        return hidden
    ...
    def evaluate(self, sess, data_manager, id_to_tag):
        ...
        account = 0
        steps_per_epoch = data_manager.len_data
        for batch in data_manager.iter_batch():
            strings = batch[0]
            labels = batch[-1]
            lengths, scores = self.run_step(sess, False, batch)
            batch_paths = self.decode(scores, lengths, trans)
            print("dev batch:{}/{}".format(account%steps_per_epoch, steps_per_epoch))
            account += 1
