...
def create_model(session, Model_class, path, bilstm_path, config, logger):
    # create model, reuse parameters if exists
    model = Model_class(config)

    ckpt = tf.train.get_checkpoint_state(path)
    bilstm_ckpt = tf.train.get_checkpoint_state(bilstm_path)
    ...
    elif bilstm_ckpt and tf.train.checkpoint_exists(bilstm_ckpt.model_checkpoint_path):
        logger.info("Restore part bilstm model parameters from %s" % bilstm_ckpt.model_checkpoint_path)
        session.run(tf.global_variables_initializer())
        model.bilstm_saver.restore(session, bilstm_ckpt.model_checkpoint_path)
    else:
